                ...
                <v-dialog scrollable v-model="dialog" max-width="900px">
                    <v-card>
                        <v-card-title class="headline primary lighten-2 pa-3" primary-title>
                            <span class="headline white--text">Export Products</span>
                        </v-card-title>
                        <v-card-text>
...
export default {
    ...
    methods: {
        getSettings() {
            ...
            this.$axios.get(this.$URLs.PRODUCTS_EXPORT_SETTINGS)
                .then((response) => {
        ...
        save() {
            ...
            return this.$axios.post(this.$URLs.PRODUCTS_EXPORT_SETTINGS, options)
                .then((response) => {
                    this.$store.dispatch("showProgress", false)
