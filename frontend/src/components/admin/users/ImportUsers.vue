                                ...
                                <v-tab-item> 
                                    <v-card flat>
                                        <v-card-text>
                                            ...
                                            <v-row dense>
                                                <v-col>
                                                    <v-alert v-if="ServerErrors.length > 0" :value="true" color="error" outlined class="pa-1">
                                                        {{ ServerErrors.join(" ") }}
                                                    </v-alert>
                                                </v-col>
                                            </v-row>
...
export default {
    ...
    methods: {
        ...
        getSettings() {
            ...
            this.$axios.get(this.$URLs.USER_IMPORT_SETTINGS)
                .then((response) => {
        ...
        save() {
            ...
            return this.$axios.post(this.$URLs.USER_IMPORT_SETTINGS, options)
                .then((response) => {
                    this.$store.dispatch("showProgress", false)
        ...
        downloadSample() {
            let url = '/storage/users_import.' + this.ImportFileFormat.toLowerCase();
            window.open(this.$ApiBaseUrl + url, '_blank')
        },

        importData() {
            if (this.ImportFile == null) {
                this.$store.dispatch("showSnackbarMessage", {message: this.$vuetify.t('$vuetify.SelectFileMsg'), code: '0'})
                return false;
            }
            ...
            this.$axios.post(this.$URLs.USER_IMPORT, data)
                .then((response) => {
                    this.$store.dispatch('showProgress', false);
                }).catch((e) => {
                    this.$store.dispatch('showProgress', false);
        ...
        rowDragStart(ev) {
            console.log("Row is being dragged..")
